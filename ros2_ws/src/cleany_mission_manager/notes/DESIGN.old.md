...
초기 MVP에서는 복잡한 VLA/VLM 기반 판단을 바로 적용하지 않고, **rule-based FSM**으로 전체 작업 흐름을 먼저 구현한다. 이후 데이터가 쌓이면 FSM 전체를 제거하는 것이 아니라, FSM 내부의 `PLAN_TASKS` 단계만 VLM/VLA 기반 Planner로 교체한다.

핵심 원칙은 다음과 같다.

> FSM은 끝까지 유지하고, Planner만 교체 가능하게 만든다.

논의에서 확정한 결정 사항은 `DECISIONS.md`에 기록한다.
...
초기 Mission Manager는 퇴실 좌석 정리 시나리오를 기준으로 다음 8개 상태로 구현한다.
...
이 FSM은 VLM/VLA가 생성하거나 수정하지 않는다.  
VLM/VLA는 `PLAN_TASKS` 상태에서만 Planner 구현체로 호출된다.
...
- mission start 수신 → `NAVIGATE_TO_TARGET`
- hardware error 감지 → `ERROR`

---

### 3.2 NAVIGATE_TO_TARGET

퇴실한 사용자의 자리 또는 관리자가 지정한 목표 지점까지 이동하는 상태.

주요 동작:

- mission request의 `seat_id` 또는 `target_pose` 확인
- Nav2 또는 mock navigation 호출
- 이동 timeout 관리
- 도착 여부 확인
...
목표 지점에 도착한 뒤 좌석과 주변 물체를 스캔하고 인식하는 상태.

주요 동작:

- 카메라/depth frame 수집
- object detection / segmentation 수행 또는 perception node 호출
- 쓰레기, 분실물 후보, 기타 물체 분류
...
- 객체 후보 있음 → `PLAN_TASKS`
- 객체 후보 없음 → `REPORT`
- confidence가 전반적으로 낮음 → `PERCEIVE` 재시도 또는 `REPORT`
...
- 유효한 skill sequence 생성 → `EXECUTE_TASKS`
- 실행 가능한 작업 없음 → `REPORT`
- planner error → `ERROR`
...
Planner가 생성한 skill sequence를 순서대로 실행하는 상태.
...
- skill 성공 → 다음 skill 또는 `RETURN_HOME`
- skill 실패 후 retry 가능 → `EXECUTE_TASKS`
...
VLMPlanner 출력은 반드시 schema validation과 Safety Guard를 통과해야 한다.  
검증에 실패하면 Mission Manager는 해당 plan을 실행하지 않고 `REPORT`, `PERCEIVE`, `ERROR` 중 하나로 전이한다.
...
```text
mission_manager_node.py   # ROS2 service/topic/action adapter
core/                     # ROS2를 모르는 순수 FSM core
```

ROS2 Node는 입출력만 담당한다. FSM core는 ROS2 타입에 직접 의존하지 않는다.
이렇게 해야 pytest로 Mission Manager 흐름을 먼저 검증하고, 이후 ROS2 wrapper를 얹을 수 있다.
...
2. VLM/VLA는 고수준 작업 계획에만 사용하며, FSM을 생성하지 않는다.
...
초기에는 rule-based FSM으로 Cleany의 전체 작업 파이프라인을 뚫고, 이후 FSM의 `PLAN_TASKS` 단계만 VLM/VLA 기반 Planner로 교체한다. FSM은 안전, 상태 관리, 재시도, 실패 로그를 위해 끝까지 유지한다.
