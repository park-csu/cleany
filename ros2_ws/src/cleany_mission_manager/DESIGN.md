...
MVP에서는 `SeatMap` 또는 `TargetResolver`를 별도 모듈로 두지 않는다.
`seat_id -> target_pose` 변환은 Navigator 내부 책임으로 둔다.
좌석/공간 map 로직이 커지면 이후 별도 `SeatMap` 또는 `TargetResolver`로 분리한다.
...
MVP에서는 Mission Manager가 `target_pose`를 직접 받지 않는다.
Navigator가 `target_id`를 내부적으로 pose에 매핑한다.

나중에 필요하면 `target_pose`, `home_pose`, `priority`, `deadline` 등을 추가한다.

### ModuleResult

Mission Manager가 호출하는 모듈은 공통 result envelope를 반환한다.
Mission Manager는 이 공통 형식을 보고 상태 전이, retry, report, error를 결정한다.
...
```text
NAVIGATE_TO_TARGET
- navigation timeout은 1회 재시도

PERCEIVE
- LOW_CONFIDENCE는 1회 재스캔
- PERCEPTION_FAIL은 1회 재시도

PLAN_TASKS
- 기본적으로 재시도하지 않음
- PLAN_EMPTY 또는 PLAN_BLOCKED는 REPORT

EXECUTE_TASKS
- GRASP_FAIL은 skill 단위로 최대 2회 재시도
- PLACE_FAIL은 skill 단위로 최대 2회 재시도
- E_STOP은 즉시 ERROR
...
```text
Task Plan
- 책상 위 물건들 중 무엇을 어떤 순서로 처리할지 결정

Skill / Motion Plan
- 선택된 물건 하나를 실제로 어떻게 접근하고, 잡고, 들어올리고, 놓고, 복귀할지 결정
```
...
MVP에서는 Planner 출력 형식을 구체적으로 고정하지 않는다.
정확한 TaskPlan 구조는 `cleany_planner` 구현 시 확정한다.
Mission Manager는 PlannerResult가 실행 가능한 high-level skill sequence를 포함하는지만 계약으로 요구한다.

Planner가 `skip`, `human_review` 같은 비실행 판단을 반환할 수는 있다.
...
Skill Executor는 필요하면 Robot Interface, motion planner, Nav2, MoveIt, LeRobot 등을 사용한다.
Mission Manager는 이 세부 실행 과정을 알지 않는다.
...
`REPORT`는 mission의 최종 결과를 정리하는 상태다.
성공뿐 아니라 실패, blocked, skip, human review 요청도 `REPORT`에서 처리한다.

MVP에서는 `human_review`를 별도 FSM 상태로 두지 않는다.
관리자 승인 flow가 필요해지면 이후 별도 상태로 분리한다.
...
failure_code: None
completed_tasks: [obj_1_collect]
skipped_tasks: [obj_2_personal_item]
failed_task: None
needs_human_review: true
summary: "Collected trash; personal item requires review."
```

MVP에서는 report를 console log 또는 file log로 남긴다.
이후 dashboard event publish 또는 backend upload로 확장한다.
